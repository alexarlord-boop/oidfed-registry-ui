 ...
 * 
 * Note: In browser context, Bun.env and process.env are not available.
 * The bundler should inject these at build time or we use fallbacks.
 */
export function getEnv(key: string, fallback: string = ''): string {
  ...
  // Check process.env (Node/Bun server-side)
 ...
 */
export const env = {
  // Auth Service - use AUTH_SERVICE_URL or AUTH_GATEWAY_URL or fallback
  AUTH_SERVICE_URL: getEnv('AUTH_SERVICE_URL') || getEnv('AUTH_GATEWAY_URL') || 'http://localhost:9000',
  
  // API - Service handles /api prefix, so base URL should not include it
