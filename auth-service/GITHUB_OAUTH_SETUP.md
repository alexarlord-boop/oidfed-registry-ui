...
This guide explains how to configure GitHub OAuth authentication for the OIDFED Registry Auth Service.
...
4. Fill in the application details:
   - **Application name**: `OIDFED Registry Auth Service (Dev)`
...
Edit the `.env` file in the `auth-service/` directory:
...
If the auth service is running in Docker:
...
2. **Redirect to GitHub**: Auth service redirects to GitHub's OAuth authorization page
...
5. **Token Exchange**: Auth service exchanges the code for an access token
6. **User Info**: Auth service fetches user profile and email from GitHub API
...
1. Create a new OAuth app with production URLs:
   ...
   - Callback URL: `https://your-auth-service.com/auth/oidc/github/callback`
...
2. Update environment variables:
   ...
   GITHUB_REDIRECT_URI=https://your-auth-service.com/auth/oidc/github/callback
   ```
...
- Restart the auth service after changing `.env`
...
- Look at auth service logs for detailed error messages

### Users redirected to wrong URL after login

- Check `FRONTEND_REDIRECT_URI` in auth service `.env`
...
GitHub OAuth adds these endpoints to the auth service:
